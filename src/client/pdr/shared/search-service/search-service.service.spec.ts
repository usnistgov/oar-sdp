import {
 ...
 describe('SearchService (mockBackend)', () => {
   ...
   describe('when searchRecords', () => {
       ...
       it('should have expected fake records (then)', async(inject([], () => {
         backend.connections.subscribe((c: MockConnection) => c.mockRespond(response));
 
         service.searchById(testid).toPromise()
         // .then(() => Promise.reject('deliberate'))
           .then(records => {
             expect(records.length).toBe(fakerecords.length,
               'should have expected no. of records');
           });
       })));
 
       it('should have expected fake records (Observable.do)', async(inject([], () => {
         backend.connections.subscribe((c: MockConnection) => c.mockRespond(response));
         let testvalue : string = '';
         service.searchRMMAny(testvalue)
           .do(records => {
             expect(records.length).toBe(fakerecords.length,
               'should have expected no. of records');
           })
       ...
       it('should be OK returning no records', async(inject([], () => {
         let resp = new Response(new ResponseOptions({status: 200, body: {data: []}}));
         backend.connections.subscribe((c: MockConnection) => c.mockRespond(resp));
         let testid: string = '';
         service.searchById(testid)
           .do(records => {
             expect(records.length).toBe(0, 'should have no records');
           })
           .toPromise();
       })));
